...
import {refractor, type Syntax, type RefractorRoot, RefractorElement} from 'refractor'
import {esql} from '..';
...
const walk = (prefix: string, node: RefractorRoot | RefractorElement): Token[] => {
  ...
  for (const child of node.children) {
    if (child.type === 'element') {
      const classNames = child.properties?.className ? Array.isArray(child.properties?.className) ? child.properties.className.join('+'): String(child.properties?.className) : '';    
